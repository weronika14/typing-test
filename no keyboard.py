...
def main():
    ...
    writingText(('time: '+timer.timer), 140, 140, 100, 50, (colour), (255,255,255), 50)
    ...
    for i in range(len(letters_on_screen)): #displays the letters on the screen indivusually
        ...
        if x < 900:
            letters_as_text.append(writingText(letters_on_screen[i], x, (ScreenHeight/2), 40, size, rect_colours[i], letter_colours[i], 50))
            x += letter_width[i]
        if line != 0:
